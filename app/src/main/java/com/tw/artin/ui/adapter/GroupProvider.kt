...
class GroupProvider (val mFragment : DeviceFragment) : BaseItemProvider<DevicesBean>() {
            ...
            .asAttachList(
                ...
                if (position == 0){
                    ...
                    getAdapter()?.data?.filter { it.type == 3 && it.group_child_name == data.group_name }?.let {
                        ...
                        if (it.isEmpty()){
                            //直接删除分组
                            mFragment.getAttachActivity().mainControl.removeGroup(data.address!!)
    ...
    private fun showInputDialog(g_name : String, address : Int){
        ...
        XPopup.Builder(context)
            ...
            .asCustom(
                ...
                CreatGroupPop(context, mFragment,g_name,str03).apply {

                    setOnSelectListener(object : CreatGroupPop.InputListener{
                        override fun onInputData(txt: String) {

                            mFragment.getAttachActivity().mainControl.editGroupName(txt,address)
                        }
                    })
                }
